...
% addpath(genpath('/Users/alimahdi/Desktop/Dissertation/MIT_evaluation_metrics/code_forMetrics'));

% read the human data from the directory stored in data_path
data_path = 'Fixation Data/';
...
img_path = ['/Users/alimahdi/Desktop/Dissertation/Fashion_and_Design/Fashion_Design_Merchandising/FDM_Dataset/Batch_',num2str(batch(index)),'/'];
...
for i = 1 : size(img_dir,1)
    ...
    if size(fix_data(imgList(i)).observer,1) ~= 0
        ...
        [fix, ~] = run_antonioGaussian(map, ppd);
        ...
        imwrite(fixMap, ['/Users/alimahdi/Desktop/Dissertation/Fashion_and_Design/Fashion_Design_Merchandising/FDM_Dataset/fixation_maps/', strng(1:end-4), '_fixMap.jpg']);
        
        pts = map(pad_x/2 + 1 : pad_x/2 + new_row, pad_y/2 + 1 : pad_y/2 + new_col);
        fixPts = uint8(pts .* 255);

        % saving the binary fixation maps
        imwrite(fixPts, ['/Users/alimahdi/Desktop/Dissertation/Fashion_and_Design/Fashion_Design_Merchandising/FDM_Dataset/fixation_maps/', strng(1:end-4), '_fixPts.jpg'])
    end
%         
end
end
